...
WangGouRESTAPI.prototype.encrypt = function (key, method,queryMethod) {
		...
		var param = queryMethod + "&" + encodeURIComponent(method, "UTF-8") + "&" + encodeURIComponent(String(orgin));
		var signingKey = new SecretKeySpec(String(key).getBytes("UTF-8"), "HmacSHA1");
		var sha256_HMAC = Mac.getInstance("HmacSHA1");
		sha256_HMAC.init(signingKey);
		var sign = sha256_HMAC.doFinal(String(param).getBytes("UTF-8"));
		var signBase64 = new Base64().encodeBase64(sign);
		return String(signBase64, "UTF-8");
};
...
WangGouRESTAPI.prototype.URLEncode = function (ru, rp) {
	...
	var url = request_url + "?" + String(request_params.toString().substring(1));
	return url;
};