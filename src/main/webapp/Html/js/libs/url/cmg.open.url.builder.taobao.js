...
TaobaoMD5.prototype.encrypt = function (app_secret) {
		...
		var bytes = String(orgin).getBytes("UTF-8");
...
TaobaoMD5.prototype.EncodeURL = function (ru, rp) {
	...
	if(null != url){
		...
		var data = rsaEncode.encryptData(String(url).getBytes("UTF-8"));
		url = URLEncoder.encode(String(Base64.encodeBase64(data)),"UTF-8")
	}
	return url;
};